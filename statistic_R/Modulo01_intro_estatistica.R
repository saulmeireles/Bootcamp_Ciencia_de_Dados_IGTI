...
rm(list=ls(all=TRUE)) # Remove objetos da memoria do R
...
hist(dados$Preco_Cafe,
     col = 'green',
     main = 'Distribuição dos Preços Praticados para  Cafe')
...
hist(dados$Preco_Cafe,
     col = 'green',
     main = 'Distribuicao dos PreÃ§os do Café')
hist(dados$Preco_Leite,
     col = 'green',
     main = 'Distribuicao dos Preçoos do Leite')

dev.off() # limpa os gráficos e volta o layout para configuracão normal
...
plot(y = dados$Vendas_Cafe,
     ...
     main = 'Relação entre o Preço e as Vendas do Cafe')

grid() # este comando adiciona linhas de grade ao grafico

# Colore os pontos em que havia promoçao naquele dia
